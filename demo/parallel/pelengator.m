...
N_start=1;
N_end=N;
panorama_start_frequency = 10000;
panorama_stop_frequency = 100000;
...
subplot(2,1,2);
hold off
plot(panorama_fscale,squelsh_line,'color','green');
hold all
plot(panorama_fscale,squelsh_base_line,'color','red');
plot(panorama_fscale,0.63*panorama_max*squelsh_detected,'color','black');
axis([panorama_fscale(1) panorama_fscale(end) 0 panorama_max]);
...
for j=1:size(station_center_frequency,1)
    ...
    modulation_mask(s_mask,s_spawn) = modulation_mask(s_mask,s_spawn) + s_current;
    m_mask = modulation_mask(s_spawn);
