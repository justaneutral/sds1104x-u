function [direction,Pxy,An,iq_accumulator,ant_color] = measurement_correlational( ...
    ...
    min_power_level, ...
    insist_AoA_return, ...
    iq_accumulator,iq_relaxation_gain_hz,ant,squelsh_latch,modulation_mask,reference_angle,fig_num,fig_hndl,fig_prm,rgi,rbi,gbi)
...
%     ((ant(3,:).*abs(ant(2,:))).^2) ...
%     ));
rg = rg*(1-alpha) + rgi*alpha;
rb = rb*(1-alpha) + rbi*alpha;
gb = gb*(1-alpha) + gbi*alpha;

fprintf("rg:%f, rb:%f, gb:%f\n", rg,rb,gb);
...
if fig_num > 0
    ...
    angle_text = sprintf('AoA = %3.1f deg.|rg%f|rb%f|gb%f',mod(direction,360),rg,rb,gb);
