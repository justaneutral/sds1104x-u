...
ViStatus osc_init(OscCtx *ctx, const char *resourceName)
{
    ...
    //ctx->acq_delay_us = (unsigned long)(1000000.0 * duration_seconds);
    ...
    if ((ViUInt32)-1==viwrite_str(instr, (ViBuf)"WFSU SP,0,NP,0,FP,0,SN,0\n")) goto fail;
    //if ((ViUInt32)-1==viwrite_str(instr, (ViBuf)"WFSU SP,1,NP,1400000,FP,0,SN,0\n")) goto fail;
    //if ((ViUInt32)-1==viwrite_str(instr, (ViBuf)"WFSU SP,100,NP,70000,FP,0,SN,0\n")) goto fail;
    usleep(5000);
...
ViStatus osc_step(OscCtx *ctx)
{
    ...
    if ((ViUInt32)-1==(retCount=viread_str(instr, BUF_CH1(ctx->buffer), __BUFFER_BYTE_LEN__))) return VI_ERROR_SYSTEM_ERROR;

    if ((ViUInt32)-1==viwrite_str(instr, (ViBuf)"C3:WF? DAT2\n")) return VI_ERROR_SYSTEM_ERROR;
    if ((ViUInt32)-1==(retCount=viread_str(instr, BUF_CH2(ctx->buffer), __BUFFER_BYTE_LEN__))) return VI_ERROR_SYSTEM_ERROR;

    if ((ViUInt32)-1==viwrite_str(instr, (ViBuf)"C4:WF? DAT2\n")) return VI_ERROR_SYSTEM_ERROR;
    if ((ViUInt32)-1==(retCount=viread_str(instr, BUF_CH3(ctx->buffer), __BUFFER_BYTE_LEN__))) return VI_ERROR_SYSTEM_ERROR;
