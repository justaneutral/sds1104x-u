function [b_mean,b_dev] = show_accumulated_station(st_id,f_shift,Fcap,Frezolution,file_name,num_frames)
    ...
    for i=1:1:NumRepetitions
        ...
        figure(2)
        alpha_median = sort(sort(correctAngleJumps180_unwrap(alphas(:,1:i))));
        alpha_median = alpha_median(ceil(end/2));
        alpha_mean = sum(sum(correctAngleJumps180_unwrap(alphas(:,1:i))))/(i*NumChannels);
        alpha_dispersion = sum(sum(correctAngleJumps180_unwrap(alphas(:,1:i)).^2-alpha_mean^2))/(i*NumChannels);
    ...
    alphas = [alphas(1,:) alphas(2,:) alphas(3,:)];
    alphas = correctAngleJumps180_unwrap(alphas);
    fid = fopen([file_name '_' st_id '_angles.txt'],'w');
    fprintf(fid,'%f\n',alphas);
    fclose(fid);
    b_mean = mean(alphas);
    b_dev = std(alphas);
