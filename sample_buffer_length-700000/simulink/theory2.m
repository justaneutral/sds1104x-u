...
for k=0:2
    ...
    sa(k+1,:) = sign(cos(cos(alpha+(k+1)*2*pi/3).*cos(alpha+(k+2)*2*pi/3)));
...
for k=0:2
    ...
    hold all
    plot(alpha*180/pi,20*A+135,'color','red');
    plot(alpha*180/pi,20*B+90,'color','green');
    plot(alpha*180/pi,20*C+45,'color','blue');
    hold off
end
