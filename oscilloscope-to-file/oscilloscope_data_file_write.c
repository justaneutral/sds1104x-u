...
#define CHANNEL_LENGTH (7000000) //(65536) //(8192)
#define PATH_FILE_NAME ("../simulink/temp_output.txt")
//#define __VERBOSE__
#undef __VERBOSE__


int oscilloscope_data_file_write(signed char *p0, signed char *p1, signed char *p2, signed char *p3, double sleep_time_ns) 
{
    FILE *file;
...
#ifdef __TEST_OSCILLOSCOPE_DATA_FILE_WRITE__
int test_oscilloscope_data_file_write()
{
    const char file_name[] = "temp_output.txt";
    int delay_nanoseconds = 1000000000.0; //100 milliseconds
    signed char a[NUM_CHANNELS*CHANNEL_LENGTH];
    int n = 1;
    while (1)
    {
